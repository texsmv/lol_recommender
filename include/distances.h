...
__device__ float d_pearson(RBTree<int, float>* rbt1, RBTree<int, float>* rbt2){
  float sum_xy = 0;
  float sum_x2 = 0;
  float sum_y2 = 0;
  float sum_x = 0;
...
__global__ void one2all_cosine_gpointer(RBTree<int, float >** dp, RBTree<int, float >** dp_user, float* d_distances, int* d_row_size, int max_users){
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if(i < max_users){
    if(d_row_size[i] != 0){
      d_distances[i] = d_pearson(dp_user[0], dp[i]);
    }
  }
}
